...
bool ShapefileGraph::Build(double minx, double miny, double maxx, double maxy) {
    ...
    for (int i = 0; i < nodes.size(); ++i) {
        ...
        vertex_map_.insert(std::make_pair(info.id_, vertex_descript));
    ...
    for (int i = 0; i < edges.size(); ++i) {
        ...
        Graph::vertex_descriptor start_vertex = vertex_map_.at(node_index[0]);
        Graph::vertex_descriptor end_vertex = vertex_map_.at(node_index[1]);
...
bool ShapefileGraph::ShortestPath(int node1, int node2, std::vector<int>& path, double& dist) {
    ...
    Graph::vertex_descriptor from_vertex = vertex_map_.at(node1);
    Graph::vertex_descriptor to_vertex = vertex_map_.at(node2);
...
void ShapefileGraph::Save(std::string filename) {
    ...
    oa & BOOST_SERIALIZATION_NVP(vertex_map_);
...
void ShapefileGraph::Load(std::string filename) {
    ...
    ia & BOOST_SERIALIZATION_NVP(vertex_map_);
    ia & BOOST_SERIALIZATION_NVP(edges_on_vertex_);
    
    DebugUtility::Print(DebugUtility::Verbose, "Loading " + boost::lexical_cast<std::string>(vertex_map_.size()) + " vertex.");
    ifs.close();
    
//    std::pair<Graph::vertex_iterator, Graph::vertex_iterator> vertex_iterator_range = boost::vertices(graph_);
//    for (Graph::vertex_iterator vertex = vertex_iterator_range.first; vertex != vertex_iterator_range.second; ++vertex) {
//        vertex_map_.insert(std::make_pair(graph_[*vertex].id_, *vertex));
//    }
}
...
std::vector<int> ShapefileGraph::GetEdgeOnVertex(int id) {
    ...
    tie(edgeIt, edgeEnd) = boost::out_edges(vertex_map_.at(id), graph_);
